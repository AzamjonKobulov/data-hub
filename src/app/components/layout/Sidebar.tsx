...
export default function Sidebar() {
  ...
  return (
    <>
      ...
      ></div>
      <aside
        className={`fixed md:sticky left-0 top-0 z-50 min-h-screen ${
          isSidebarOpen
            ? "w-70 shrink-0"
            : "w-fit md:max-h-250 -translate-x-full md:translate-x-0"
        } h-full flex flex-col justify-between bg-neutral-800 border-r border-neutral-500/[8%] transition-all duration-500 ease-in-out md:transition-none pb-20 px-5 pt-5`}
        ref={sideBarRef}
      >
        <div>
          {/* Logo */}
              ...
              <li key={l.label}>
                <Link
                  ...
                  className={`group flex items-center gap-2 ${
                    pathname === l.path
                      ? "bg-neutral-500/[12%] text-white"
                      : "text-neutral-500"
                  } rounded-xl hover:bg-neutral-500/[12%] hover:text-white smooth-200 p-2`}
                >
                  <l.icon
                    className={`${
                      pathname === l.path
                        ? "fill-brand-500"
                        : "fill-neutral-500"
                    }  group-hover:fill-brand-500 transition-colors duration-200`}
                  />
                  {isSidebarOpen && <span>{l.label}</span>}
                </Link>
              </li>
            ))}
          </ul>
